...
get_pwin <- function(modout,
  ...
  if (is.null(control) || !(control %in% names)) {
    ...
    for (xx in allpairs(n)) {
      ...
      larger <- c(larger, names[i])
      smaller <- c(smaller, names[j])
    }
  } else {
    ...
    for (j in 1 : n)
      if (i < j) {
        ...
        larger <- c(larger, names[j])
        smaller <- c(smaller, names[i])
        k <- k + 1
      }
  }

  colnames(out) <- out_names
  # attached for callers (e.g. table_pwin()) that want the two algorithm
  # names of each pair as separate, programmatically usable fields rather
  # than parsing them back out of the "A > B" colnames
  attr(out, "larger") <- larger
  attr(out, "smaller") <- smaller
  return(out)
}
