...
The `bbtcomp` package requires the `cmdstanr` package, which is not on
...
Installing `bbtcomp` installs the `cmdstanr` R package, but it
cannot install or verify CmdStan itself (the compiled Stan
backend) -- CmdStan is not an R package, so R’s installers have
no way to know about it. After installing, run this once to
confirm CmdStan is actually set up:

    check_setup()
    # CmdStan found at: /path/to/cmdstan-x.y.z

If CmdStan isn’t found, `check_setup()` prints (or, with
`check_setup(raise_on_error = TRUE)`, stops with) clear setup
instructions instead of a cryptic error later. `bbtcomp()` /
`mcmcbbt()` also run this check automatically and fail fast with
the same message if CmdStan is missing.
...
-   two matrices, where the column names are the same in both, the
    first matrix containing the mean (across the cross-validation
    folds) of the measure for each algorithm and data set, and the
    second matrix, the standard deviation of the measures (across the
    same folds). In this case the algorithm can compute the non-paired
    version of the local ROPE.

The algorithm assumes that higher values of the metric are better, as
with accuracy, AUC, F1, and other metrics. But if the metric is an
error measure, such as RMSE, MAE, or others, the user should multiply
these values by -1 (so that higher values -- negative but close to 0 --
are better). If the algorithm did not run for that fold, or for that
data set, the entry should be `NA`.

The `bbtcomp` package comes with a data set comparing 16 algorithms on
132 data sets (as described in the paper). The data set is called `ll`
...
potentially multiple measures for each data set (for multiple folds).
...
This data has only a single measure per algorithm (column) and per
data set (each row, implicitly). This format does not allow one to
use the local ROPE (discussed below).
...
CmdStan, which serves as the interface between bbtcomp and Stan (the
MCMC engine under the hood of bbtcomp), uses files as an intermediary
between the two processes. First, the Bayesian model of BBT is
translated into C++ and then compiled and saved into a file. The
sampling process then saves the results as .csv files (one per chain).
All these files are stored in an auxiliary folder.

This auxiliary folder can be set using

    options(bbtcomp.dir = "~/.bbtcomp")

In this case, the auxiliary folder is “~/.bbtcomp”. If the folder is
not set, then bbtcomp will use a temporary folder that will be deleted
as soon as the R session is terminated.

The advantage of setting the auxiliary folder with the `options`
command is that the compiled Stan program is stored there and there is
no need to compile it again in another R session using bbtcomp. The
disadvantage is that each call to `bbtcomp` generates 4 .csv files, and
the user should erase these .csv files from time to time.

The advantages and disadvantages of using a temporary folder are the
opposite of those of the fixed auxiliary folder: a new compilation of
the Stan model will be necessary each time `bbtcomp` is called in a new
R session, but there is no need to delete the .csv files from time to
time.
...
Although the ss data has 4 entries for each data set -- the results on
4 different test sets (4-fold cross-validation) -- the results
displayed above compute the mean for each algorithm and data set and
perform the BBT on the mean results.
...
the results for the folds. The paper calls it a local ROPE, and discusses
...
`cmdstanr` uses a directory to store two important pieces of
information from the sampling process. The first is the compiled Stan
model: the first time one calls `bbtcomp`, the Stan model is compiled
and stored in this directory. The directory is set by the `dir`
parameter of the `bbtcomp` function, and the default is `.bbtcomp` in
the current directory.

The second piece of data stored in the directory is the .csv files
containing the samples themselves. By default, 4 chains are sampled in
parallel, so the sampling process generates 4 .csv files with the
results. The samples themselves are part of the bbt model object
returned by `bbtcomp`, and this object is passed to the functions that
generate the tables and the graphs. By default, `bbtcomp` deletes the
.csv files in the directory since they tend to grow large (each call to
`bbtcomp` generates 4 new large .csv files). But the convergence check
performed by the `cmdstan_diagnose` function reads those files, so to
run the convergence tests one must disable the cleanup of the .csv
files.
...
The posterior predictive checks of the model in relation to the data
can be generated by
...
The table of wins and losses (a wintable) for all algorithms can be
accessed as the `wintable` component of the model returned by `bbtcomp`.
To print the table of victories and losses with the ties shown
explicitly (i.e., unprocessed), use:
...
The Python implementation needs the `cmdstanpy` package, so please
install it following the steps described in
<https://mc-stan.org/cmdstanpy/>.

The Python package lives in the `bbtcomp_python` directory as an
installable pip package. Install it (in editable mode, while
developing) with:
...
The Python package mirrors the full R package: `bbtcomp`,
`make_wintable`, `mcmcbbt`, `table_pwin`, `table_ppc`,
`table_wintable`, `convergence_check`, `get_waic`, `get_loo`,
`plot_pwin`, `plot_ppc`, and `load_ll()` for the bundled `ll` data set.

    from bbtcomp import bbtcomp, table_pwin, load_ll

    ll = load_ll()

`bbtcomp()` takes the same parameters, data formats, and so on, as the
R version. A few differences:

-   to indicate that there is no `dbcol`, use `dbcol=None` (or
    `dbcol=-1`);

-   to control the auxiliary folder, set the `output_dir` parameter.
    If left unset, `bbtcomp` uses a temporary folder that is deleted
    as soon as the Python session ends.
